...
const galleryItems = [
  ...
  { id: 24, src: '/images/gallery/northern-lights/northern-lights-aurora-colour-realism.png', categories: ['northern-lights', 'nature-landscapes'],            title: 'Northern Lights Aurora' },
  // Row 5
      ...
      <section className="section container">
        ...
        <motion.div className="gallery-masonry" layout>
          {filteredItems.map((item, i) => (
            <motion.div
              key={item.id}
              className={`gallery-masonry-item${item.zoom === true ? ' gallery-masonry-item--zoomed' : item.zoom === 'sm' ? ' gallery-masonry-item--zoomed-sm' : item.zoom === 'md' ? ' gallery-masonry-item--zoomed-md' : item.zoom === 'lg' ? ' gallery-masonry-item--zoomed-lg' : ''}${item.posClass ? ` gallery-masonry-item--${item.posClass}` : ''}`}
              layout
